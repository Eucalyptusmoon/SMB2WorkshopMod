...
int main(int argc, char **argv)
{
    ...
    const std::string lstFilename{argv[2]};
    ...
    auto findSymbolSectionAndOffset = [&](const std::string &name, int &sectionIndex, int &offset)
    {
        ...
        fprintf(stderr, "Failed to find symbol in ELF: '%s'\n", name.c_str());
        exit(1);
    };
    ...
    for (const auto &section : relocationSections)
    {
        ...
        if (writtenSections.find(relocatedSection) != writtenSections.end())
        {
            ...
            for (int i = 0; i < relocations.get_entries_num(); ++i)
            {
                ...
                {
                    fprintf(stderr, "Unable to find symbol %u in symbol table!\n", static_cast<uint32_t>(symbol));
                    return 1;
                }
                ...
                if (sectionIndex)
                {
                    ...
                    {
                        fprintf(stderr, "Relocation from section '%s' offset %" PRIx64 " against symbol '%s' in unwritten section '%s'\n",
