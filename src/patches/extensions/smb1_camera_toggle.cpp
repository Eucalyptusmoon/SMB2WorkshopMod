...
void init_main_loop() {
    ...
    // Challenge
    patch::write_branch_bl(relutil::relocate_addr(0x80273e5c), reinterpret_cast<void*>(remove_how_to));
    patch::write_nop(relutil::relocate_addr(0x80273df0));
    patch::write_nop(relutil::relocate_addr(0x80273e00));
    patch::write_nop(relutil::relocate_addr(0x80273e58));
    patch::write_nop(relutil::relocate_addr(0x80273e6c));
    // Practice
    patch::write_branch_bl(relutil::relocate_addr(0x80274148), reinterpret_cast<void*>(remove_how_to));
    patch::write_nop(relutil::relocate_addr(0x802740dc));
    patch::write_nop(relutil::relocate_addr(0x802740ec));
    patch::write_nop(relutil::relocate_addr(0x80274144));
    patch::write_nop(relutil::relocate_addr(0x80274158));
    // Story
    patch::write_branch_bl(relutil::relocate_addr(0x802747e0), reinterpret_cast<void*>(remove_how_to));
    patch::write_nop(relutil::relocate_addr(0x80274774));
    patch::write_nop(relutil::relocate_addr(0x80274784));
    patch::write_nop(relutil::relocate_addr(0x802747dc));
    patch::write_nop(relutil::relocate_addr(0x802747f0));
    ...
    patch::write_branch_bl(relutil::relocate_addr(0x80273878), reinterpret_cast<void*>(fix_on_off));
}
...
void tick() {
    ...
    if (mkb::pausemenu_type == mkb::PMT_CHALLENGE) {
        if ((mkb::g_some_status_bitflag_maybe_pause_related & 4) != 0) {
            patch::write_word(relutil::relocate_addr(0x8032a914), PPC_INSTR_CMPWI(PPC_R27, 9));
        }
        else {
            patch::write_word(relutil::relocate_addr(0x8032a914), PPC_INSTR_CMPWI(PPC_R27, 1));
        }

        if (mkb::g_current_pause_menu_entry_count == 5) {
            // Special handler for CM retry
            patch::write_word(relutil::relocate_addr(0x8032a91c), PPC_INSTR_CMPWI(PPC_R28, 3));
            patch::write_word(relutil::relocate_addr(0x80273678), PPC_INSTR_CMPWI(PPC_R0, 3));
        }
        else {
            patch::write_word(relutil::relocate_addr(0x8032a91c), PPC_INSTR_CMPWI(PPC_R28, 2));
            patch::write_word(relutil::relocate_addr(0x80273678), PPC_INSTR_CMPWI(PPC_R0, 2));
        }

        patch::write_word(relutil::relocate_addr(0x80273664), PPC_INSTR_CMPWI(PPC_R0, 1));
        ...
        if ((mkb::g_some_status_bitflag_maybe_pause_related & 4) != 0) {
            patch::write_word(relutil::relocate_addr(0x8032a914), PPC_INSTR_CMPWI(PPC_R27, 14));
        }
        else {
            patch::write_word(relutil::relocate_addr(0x8032a914), PPC_INSTR_CMPWI(PPC_R27, 6));
        }

        patch::write_word(relutil::relocate_addr(0x8032a91c), PPC_INSTR_CMPWI(PPC_R28, 3));
        patch::write_word(relutil::relocate_addr(0x80273664), PPC_INSTR_CMPWI(PPC_R0, 6));
        patch::write_word(relutil::relocate_addr(0x80273678), PPC_INSTR_CMPWI(PPC_R0, 3));
        ...
        if ((mkb::g_some_status_bitflag_maybe_pause_related & 4) != 0) {
            patch::write_word(relutil::relocate_addr(0x8032a914), PPC_INSTR_CMPWI(PPC_R27, 10));
        }
        else {
            patch::write_word(relutil::relocate_addr(0x8032a914), PPC_INSTR_CMPWI(PPC_R27, 2));
        }

        patch::write_word(relutil::relocate_addr(0x8032a91c), PPC_INSTR_CMPWI(PPC_R28, 3));
        patch::write_word(relutil::relocate_addr(0x80273664), PPC_INSTR_CMPWI(PPC_R0, 2));
        patch::write_word(relutil::relocate_addr(0x80273678), PPC_INSTR_CMPWI(PPC_R0, 3));
        ...
        patch::write_word(relutil::relocate_addr(0x8032a914), PPC_INSTR_CMPWI(PPC_R27, 4));
        patch::write_word(relutil::relocate_addr(0x8032a91c), PPC_INSTR_CMPWI(PPC_R28, 1));
        patch::write_word(relutil::relocate_addr(0x80273664), PPC_INSTR_CMPWI(PPC_R0, 4));
        patch::write_word(relutil::relocate_addr(0x80273678), PPC_INSTR_CMPWI(PPC_R0, 1));
