...
void init_main_loop() {
    ...
    // BAD-BOON's Base
    patch::write_word(relutil::relocate_addr(0x804551B8), 0xffffffff);
    patch::write_word(relutil::relocate_addr(0x80455208), 0xffffffff);
    // Overwrite Dr. BAD-BOON's Base's display/tick functions with
    // the defaults
    patch::write_word(relutil::relocate_addr(0x804749C0),
                      reinterpret_cast<u32>(relutil::relocate_addr(0x802E11BC)));
    patch::write_word(relutil::relocate_addr(0x80474868),
                      reinterpret_cast<u32>(relutil::relocate_addr(0x802E111C)));
}
